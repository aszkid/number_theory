...
The bottom line is that unique factorization does \emph{not} hold in the ring of integers of $\bbQ(\sqrt{-5})$, which is bad news. If we want to do arithmetic in $\calO_K$ the same way we do arithmetic in $\bbZ$, unique factorization into primes is a must. Eduard Kummer proposed the idea of ``ideal numbers'', in order to replace the not-so-useful notion of  irreducibles. The goal is to define ``ideal prime numbers'' $\frakp_1$, $\frakp_2$, $\frakp_3$ and $\frakp_4$ for which
\begin{align*}
...
\begin{proof}
	...
	\]
	Writing $A=(x\delta_{ij} - a_{ij})$ we get $A(\alpha_1,\dots,\alpha_n)^t=0$, so with $d=\det(A)$ we have $d\alpha_1=\cdots=d\alpha_n=0$, which implies that $d=0$. Hence $x$ solves the monic polynomial $f(X)=\det(X\delta_{ij}-a_{ij})$ with coefficients in $\calO$, i.e. $x$ is integral so $x\in\calO$. The choice of $x$ was arbitrary, so $\frakp\inv=\calO$, a contradiction.
...
\begin{thm}[Chinese remainder theorem -- Neukirch 3.6]
...
\begin{proof}
	...
	Note that $a\in\calO$ is in the kernel if $a\equiv 0\Mod{\fraka_i}$ for all $i$, i.e. $a\in\bigcap_i\fraka_i$. Reverse inclusion is clear, so the kernel of this homomorphism is exactly $\fraka$. It suffices to show that the map is surjective to apply the first isomorphism theorem, which follows by an induction argument.
\end{proof}
