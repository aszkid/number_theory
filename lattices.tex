...
We now restrict our attention to the real vector space $K_\bbR = K_\bbC^+ = \left[\prod_\tau\bbC\right]^+$, consisting of the $G(\bbC\mid\bbR)$-invariant, i.e. conjugation-invariant points of $K_\bbC$; that is, $(z_\tau)\in K_\bbC$ for which $z_{\overline{\tau}}=\overline{z}_\tau$. Indeed, for such a point we have $(\overline{z})_{\overline{\tau}}= \overline{\overline{z}}_\tau = z_\tau$. (...\todo{More properties, scalar product})

On $K_\bbR$, we have $\Tr\circ F = F\circ\Tr$, so 

We call the euclidean space $K_\bbR$ the \emph{Minkowski space} of $K$, its scalar product the \emph{canonical metric}, and the associated Haar measure the \emph{canonical measure}. We also have that (..\todo{elaborate}) the trace restricted to $K_\bbR$ precomposed with $j:K\to K_\bbR$
